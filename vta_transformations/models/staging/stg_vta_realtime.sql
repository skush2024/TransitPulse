-- models/staging/stg_vta_realtime.sql
...
cleaned as (
    select
        cast(timestamp as timestamp) AT TIME ZONE 'UTC' as ping_timestamp,
        ...
        cast(speed as numeric)         as vehicle_speed_mps,  
        cast(current_stop_sequence as integer) as current_stop_sequence
    from source
    where cast(timestamp as timestamp) AT TIME ZONE 'UTC' >= now() - interval '48 hours'
)
